    ...
    {cards.map((plan) => (
      ...
      >
        <div className="flex flex-col items-center mb-2 sm:mb-4 mt-1 sm:mt-2 w-full">
          <span className="text-[#232946] font-bold text-xl mb-2 text-start w-full">{plan.title}</span>
                      <div className="flex items-start mb-2 sm:mb-4 w-full">
            <span className="text-pink-600 text-3xl sm:text-3xl md:text-5xl font-bold">{plan.currency}{plan.price}</span>
            <span className="text-gray-500 text-sm sm:text-base font-medium ml-1 mb-1">/mo</span>
          </div>
          <button className="w-full bg-[#0a1747] text-white font-semibold py-3 mb-2 sm:mb-4 hover:bg-[#03206B] transition text-xs sm:text-lg">
            {plan.buttonText}
          </button>
        </div>
