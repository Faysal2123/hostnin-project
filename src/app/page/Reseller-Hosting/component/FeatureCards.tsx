...
const FeatureCards: React.FC<FeatureCardsProps> = ({ cards }) => (
  <div className="w-full flex flex-col sm:flex-row gap-1 sm:gap-6 justify-center items-stretch relative z-10 overflow-hidden">
    {cards.map((plan) => (
      <div
        key={plan.title}
        className="relative w-full max-w-[400px] sm:max-w-sm mx-auto bg-white rounded-2xl shadow-xl flex flex-col items-center px-3 sm:px-4 md:px-6 py-2.5 sm:py-6 md:py-8 border border-gray-200 mt-4 sm:mt-0"
        
      >
        <div className="flex flex-col items-center mb-2 sm:mb-4 mt-1 sm:mt-2 w-full">
          <span className="text-[#232946] font-bold text-xl mb-2 text-center w-full">{plan.title}</span>
                      <div className="flex items-end justify-center mb-2 sm:mb-4 w-full">
            <span className="text-pink-600 text-3xl sm:text-3xl md:text-4xl font-bold">{plan.currency}{plan.price}</span>
            <span className="text-gray-500 text-sm sm:text-base font-medium ml-1 mb-1">/mo</span>
          </div>
        ...
        <div className=" items-start  gap-1 mt-1 sm:mt-2 mb-1 w-full">
          <span className="text-slate-700 font-bold text-[15px] sm:text-sm cursor-pointer select-none flex items-center">
