    ...
    <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center items-stretch">
      {cards.map((card) => (
        ...
        >
          ...
          <span className="font-bold text-lg xs:text-xl sm:text-2xl text-[#03206B] mb-2 block text-left" style={{ fontFamily: '"Mulish", sans-serif' }}>{card.title}</span>
          <p className="text-gray-600 text-sm xs:text-base sm:text-base relative z-10 text-left" style={{ fontFamily: '"Mulish", sans-serif' }}>{card.desc}</p>
        </div>
      ))}
    </div>
  </div>
