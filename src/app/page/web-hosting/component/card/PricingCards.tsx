...
const iconMap: Record<string, React.ReactNode> = {
  basic: (
    <span className="flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-blue-100">
      <MdDescription size={16} className="text-blue-600 sm:text-xl md:text-2xl" />
    </span>
  ),
  starter: (
    <span className="flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-yellow-100">
      <MdStars size={16} className="text-yellow-500 sm:text-xl md:text-2xl" />
    </span>
  ),
  pro: (
    <span className="flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-blue-50">
      <MdRocketLaunch size={16} className="text-blue-500 sm:text-xl md:text-2xl" />
    </span>
  ),
  ultimate: (
    <span className="flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-purple-100">
      <MdSecurity size={16} className="text-purple-600 sm:text-xl md:text-2xl" />
    </span>
  ),
};
      ...
      <div className="w-full max-w-7xl">
        ...
        <div className="w-full flex flex-col lg:flex-row gap-4 sm:gap-6 justify-center items-stretch mt-8 sm:mt-10">
          {plans.map((plan) => (
            <div
              key={plan.title}
              className={`relative w-full max-w-sm mx-auto bg-white rounded-2xl shadow-xl flex flex-col items-center px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 transition-all duration-300 border-2 ${
            ...
            >
              ...
              <div className="flex flex-col items-center mb-3 sm:mb-4 mt-2">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[#232946] font-bold text-sm sm:text-base md:text-lg">{plan.title}</span>
                </div>
                {plan.description && (
                  <div className="text-gray-500 text-xs sm:text-sm text-center mb-1 min-h-[32px] sm:min-h-[38px]">
                    {plan.description}
                  </div>
                )}
                {plan.savingsText && (
                ...
                <div className="flex items-start justify-center gap-1 mb-2 mt-4 sm:mt-5">
                  ...
                  <span className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold text-[#232946]">
              ...
              <button
                className={`w-full mt-2 mb-2 py-2 sm:py-3 rounded font-semibold text-sm sm:text-base md:text-lg transition ${
                  plan.highlight
