...
const FeatureCards = ({ cards, bottom = "-180px" }: FeatureCardsProps) => (
  <div
    className="absolute left-1/2 transform -translate-x-1/2 z-20 w-full max-w-7xl px-4  md:px-0"
    ...
    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 md:gap-6 justify-center items-stretch">
      {cards.map((card) => (
        ...
        >
          ...
          <span className="font-bold text-lg sm:text-xl md:text-2xl text-[#03206B] mb-2 sm:mb-3 block text-left" style={{ fontFamily: '"Mulish", sans-serif' }}>
            {card.title}
          </span>
          <p className="text-gray-600 text-sm sm:text-base relative z-10 text-left leading-relaxed" style={{ fontFamily: '"Mulish", sans-serif' }}>
            {card.desc}
          </p>
        </div>
      ))}
